  ...
  <div class="main-box">
    ...
    <div class="left">
      <?php foreach ($children as $sub1) { ?>
        <a href="<?php echo $sub1->sub ? 'javascript:' : $webRoot . "article/c" . $sub1->id . ".html"; ?>" class='sub1'>
        ...
        <?php foreach ($sub1->sub as $sub2) { ?>
          ...
          <a href="<?php echo $webRoot . "".$sub2->abbr."/c" . $sub2->id . ".html"; ?>" class='sub2'>
            <?php echo $sub2->name; ?>
          </a>
          <?php else:?>
          <a href="<?php echo $webRoot . "article/c" . $sub2->id . ".html"; ?>" class='sub2'>
            <?php echo $sub2->name; ?>
          </a>
          <?php endif;?>
        <?php } ?>
    ...
    <div class="right">
        ...
        <div class="mid">
          ...
          <div class="block" style="height: auto">
            ...
            <div class="ctgs">
              ...
              ?>
                <a class="ctg-sub2" href="<?php echo $webRoot . 'index.php/article/c' . $sub2_id . '.html'; ?>">
                  <?php echo $sub2->name; ?>
                </a>
              <?php } ?>
            </div>
      ...
      <?php }else{ ?>
  		  ...
  		  <?php if(in_array($category->id, array(52,53,56,57,58,59,80))) {?>
...
<!--  	              <td align="center">--><?php //echo $size.'KB';?><!--</td>-->
  		  ...
  		  <?php  } elseif(in_array($category->id, array(19))) {?>
  	            ...
  	            <div class="bgtz">
	  	            <ul>
	  	            <?php foreach($articlelist as $article):?>
	  	            	<li><span><?php echo html::a(inlink('view', "id=$article->id", "category={$article->category->alias}&name=$article->alias"), $article->title);?></span><span style="float:right;"><?php echo substr($article->editedDate, 0, 10);?></</span></li>
	  	            <?php endforeach;?>
	  	            </ul>
  	            </div>
	  		  	...
	  		  	<ul>
		  		  	...
		  		  	<li>
						  ...
						  <tr>
						    ...
						    <td valign="top" colspan="2" height="20"><?php echo html::a($url, $article->title);?></td>
						  </tr>
						  <tr>
						    <td width="15%" height="80" valign="top">推荐产品：</td>
							<td valign="top">
								<?php foreach($this->loadModel('article')->getRelevanceProduct($article->id) AS $product):?>
			                    <?php echo '<div style="padding:0 10px 0 10px;"><a href="/product/'.$product->id.'.html" target="_blank">'.$product->name.'</a></div>';?>
			                    <?php endforeach;?>
							</td>
						  </tr>
						</table>
      ...
      <?php }elseif ($category->grade == 3) { ?>
        ...
        ?>
          ...
          <div class="name"><?php echo $category->desc; ?></div>
		  <?php if(in_array($category->id, array(52,53,56,57,58,59,80,83))) {?>
	  		  ...
	  		  <div class="peixun-js">
	  		  	<ul>
		  		  	...
		  		  	<li>
						  ...
						  <tr>
						    <td valign="top" align="center"><?php echo html::a('/index.php?m=article&f=read&fid='.$file[0]->id, $article->title, "target='_blank' title='{$file[0]->title}'");?></td>
						  </tr>
						  <tr>
	  		  ...
	  		  <div class="peixun-sp">
	  		  	<ul>
		  		  	...
		  		  	<li>
						  ...
						  <tr>
						    <td valign="top" align="center"><?php echo html::a($article->link? $article->link : '/index.php?m=article&f=videodetail&aid='.$article->id, $article->title, "target='_blank' title='{$file[0]->title}'");?></td>
						  </tr>
						</table>
		  		  	</li>
		  		  	<?php endforeach;?>
...
$(function(){
	$('.sw-search').click(function() {
		window.location.href = '/article/c<?php echo $category->id;?>.html?searchWord='+$('.searchWord').val();
	});
});
</script>
