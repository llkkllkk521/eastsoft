...
class navModel extends model
{
    ...
    public function getNavs($type = 'desktop_top')
    {
        ...
        foreach($navs as $nav)
        {
            ...
            /*if($nav->article) {
                $category = $this->loadModel('tree')->getByID($nav->article, 'article');
                $nav->title = $category->name;
            }
            if($nav->product) {
                $category = $this->loadModel('tree')->getByID($nav->product, 'product');
                $nav->title = $category->name;
            }*/
            ...
            foreach($nav->children as $grade2Nav)
            {
                ...
                if($grade2Nav->article) {
                    $category = $this->loadModel('tree')->getByID($grade2Nav->article, 'article');
                    $grade2Nav->title = $category->name;
                }
                if($grade2Nav->product) {
                    $category = $this->loadModel('tree')->getByID($grade2Nav->product, 'product');
                    $grade2Nav->title = $category->name;
                }
                ...
                foreach($grade2Nav->children as $grade3Nav)
                {
                    ...
                    if($grade3Nav->article) {
                        $category = $this->loadModel('tree')->getByID($grade3Nav->article, 'article');
                        $grade3Nav->title = $category->name;
                    }
                    if($grade3Nav->product) {
                        $category = $this->loadModel('tree')->getByID($grade3Nav->product, 'product');
                        $grade3Nav->title = $category->name;
                    }
                }
            }
