...
class surveyModel extends model {
    ...
    public function exportExcel($sid) {
        ...
        $_survey = $this->dao->select('user_id, survey_id, addedDate')->from(TABLE_SURVEY_CONTENT)
                  ->where('id')->eq($sid)
                  ->fetch();

        $surveyInfo = $this->dao->select('title')->from(TABLE_SURVEY)
                  ->where('id')->eq($_survey->survey_id)
                  ->fetch();

        $userInfo = $this->dao->select('account, utype')->from(TABLE_USER)
            ...
            foreach($_surveys AS $_v) {
                ...
                if($survey->title) {
                    $account = $this->dao->select('*')->from(TABLE_USER)
                                ->Where('id')->eq($_v->user_id)
                                ->fetch();
                    //error_log($surveyInfo->title."\r\n", 3, '/home/test/eastsoft/www/data/1.log');
