...
class userModel extends model
{
    ...
    public function update($account)
    {
        ...
        $user = fixer::input('post')
            ...
            ->get();
        $user->application = implode(',', $_POST['application']);
    ...
    public function getSurvey($account) {
    	if($account) {
    		$user = $this->dao->select('id, account, utype')->from(TABLE_USER)
    		->where('account')->eq($account)
    		->fetch();
    		$s = $this->dao->select('*')->from(TABLE_SURVEY)
    		->where('type')->eq($user->utype)
    		->andWhere('status')->eq('on')
    		->fetch();
    		$beginDate = strtotime($s->beginDate);
    		$endDate = strtotime($s->endDate);
    ...
    public function saveSurveyOneStop($account) {
    	if($_POST) {
    		$data = array();
    		$now = helper::now();
    		$data['addedDate'] = $now;
    		$user = $this->dao->select('id, utype')->from(TABLE_USER)
    		->where('account')->eq($account)
    		->fetch();
    		...
    		$sc = $this->dao->select('*')->from(TABLE_SURVEY_CONTENT)
    		->where('type')->eq('one')
    		->andWhere('user_id')->eq($user->id)
    		->fetch();
    		$addedDate = strtotime($sc->addedDate);
    		$beginDate = strtotime($s->beginDate);
    		$endDate = strtotime($s->endDate);
    		
    		if(time() >= $beginDate && time() <= $endDate) {
	    		if($sc) {
	    			if($addedDate >= $beginDate && $addedDate <= $endDate) {
	    				
	    			} else {
	    				$data['user_id'] = $user->id;
	    				$data['survey_id'] = $s->id;
	    				$data['content'] = str_replace('|', '', $_POST['surveyone'][0]).'|'.str_replace('|', '', $_POST['surveyone'][1]).'|'.implode(',', $_POST['myd']).'|'.str_replace('|', '', $_POST['surveyone'][2]);
	    				$data['type'] = 'one';
	    				$this->dao->insert(TABLE_SURVEY_CONTENT)
	    				->data($data)
	    				->exec();
	    			}
	    		} else {
    ...
    public function getSurveyAll($account) {
    	if($account) {
    		$user = $this->dao->select('id, account, utype')->from(TABLE_USER)
    		->where('account')->eq($account)
    		->fetch();
    ...
    public function getSurveyList($theme, $parent) {
    	if($theme) {
    		$ss = $this->dao->select('*')->from(TABLE_SURVEY)
    		->where('theme')->eq($theme)
     ...
     * @param unknown $sid
     */
    public function getSurveyContentType($sid, $sctid) {
    	$_mustinput = array(
    			'不太满意', 
    			'非常不满意', 
    			'3次', 
    	...
    	if($sctid) {
    		...
    		->where('id')->eq($sctid)
    		->fetch();
    		$_sct = explode('|', $sct->title);
    		$str = '';
    		foreach($_sct AS $k => $v) {
    			if($sct->type == 'radio') {
    				$str .= '<input type="radio" name="radio_'.$sid.'" value="'.$v.'" '.(in_array($v, $_mustinput) ? 'class="showipt"' : 'class="hideipt"').' />'.$v.'&nbsp;&nbsp;';
    			} elseif($sct->type == 'checkbox') {
    				$str .= '<input type="checkbox" name="checkbox_'.$sid.'" value="'.$v.'" '.(in_array($v, $_mustinput) ? 'class="showipt"' : 'class="hideipt"').' />'.$v.'&nbsp;&nbsp;';
    			} elseif($sct->type == 'text') {
    				$str = '<input type="text" name="text_'.$sid.'" value="'.$v.'" />';
    			}
    		}
    		
    		$se = $this->dao->select('*')->from(TABLE_SURVEY)
    		->where('id')->eq($sid)
    		->fetch();
    		
    		return $str.'&nbsp;&nbsp;<input size="30" type="hidden" name="mi_'.$sid.'" df="" class="mi-'.$sid.' mi"  placeholder="'.$se->explain.'" />';
    	}
    }
    
    public function saveSurveyContentAll($uid) {
    	...
    	->fetch();
    	$dataone['user_id'] = $uid;
    	$dataone['survey_id'] = $s->id;
    	$dataone['content'] = $_POST['surveyone'];
    	$now = helper::now();
    	$dataone['addedDate'] = $now;
    	$dataone['type'] = 'one';
    	$this->dao->insert(TABLE_SURVEY_CONTENT)->data($dataone)->exec();
    	//-end
    	
    	$all = explode(',', $_POST['surveyall']);
    	foreach($all AS $k => $v) {
    		$sur1 = explode('&', $v);
    		$sur2 = explode('_', $sur1[0]);
    		$sc = $this->dao->select('*')->from(TABLE_SURVEY_CONTENT)
    		->where('user_id')->eq($uid)
    		->andWhere('survey_id')->eq($sur2[1])
            ->andWhere('type')->eq('two')
            ->andWhere('addedDate')->eq($now)
    		->fetch();
    		if(!$sc->id) {
    			$data['user_id'] = $uid;
    			$data['survey_id'] = $sur2[1];
    			$data['value'] = $sur1[1];
    			$data['content'] = $sur1[2];
    			$data['type'] = 'two';
    			$data['addedDate'] = $now;
    			$this->dao->insert(TABLE_SURVEY_CONTENT)->data($data)->exec();
    		}
    	...
    	echo json_encode(array('result' => 'success', 'message' => '提交成功！', 'locate' => '/index.php/user-surveyonestep.html'));
    }
     ...
     * @param unknown $uid
     */
    public function checkUserAnswer($uid) {
    	...
    	if($uid) {
    		...
    		if($s->id) {
    			$sc = $this->dao->select('*')->from(TABLE_SURVEY_CONTENT)
    			->where('user_id')->eq($uid)
    			->andWhere('survey_id')->eq($s->id)
    			->andWhere('type')->eq('one')
    			->fetch();
    			...
    			if(time() >= $beginDate && time() <= $endDate) {
    				if($sc->id) {
    					if($addedDate < $beginDate || $addedDate > $endDate) {//不在调查时间段的都可以答题
    						return $s;
    					}
    				} else {
    					return $s;
    ...
    public function userExport() {
    	...
    	$pe = explode('user', dirname(__FILE__));
    	require_once $pe[0] . 'survey/PHPExcel/PHPExcel.php';
    
    	$objPHPExcel = new PHPExcel();
    
    	$objPHPExcel->getActiveSheet()->setCellValue('A1', '真实姓名');
    	...
    	$objPHPExcel->getActiveSheet()->setCellValue('E1', '身份');
    	$objPHPExcel->getActiveSheet()->setCellValue('F1', '地区');
    	$objPHPExcel->getActiveSheet()->setCellValue('G1', '地址');
    	$objPHPExcel->getActiveSheet()->setCellValue('H1', '职位');
    	$objPHPExcel->getActiveSheet()->setCellValue('I1', '公司/组织');
    	$objPHPExcel->getActiveSheet()->setCellValue('J1', '邮编');
    	$objPHPExcel->getActiveSheet()->setCellValue('K1', '主要产品或服务');
    	$objPHPExcel->getActiveSheet()->setCellValue('L1', '感兴趣的应用');
    	$objPHPExcel->getActiveSheet()->setCellValue('M1', '其他');
    	$objPHPExcel->getActiveSheet()->setCellValue('N1', '更新通知');
    	$objPHPExcel->getActiveSheet()->setCellValue('O1', '注册日期');
    	...
    	if($users) {
    		...
    		$membertype['zd'] = '个人/公司用户';
    		$membertype['dls'] = '代理/方案公司';
    		
    		$informupdateList['month'] = '每月定期通知';
    		$informupdateList['week'] = '每周定期通知';
    		$informupdateList['timely'] = '即时接受通知';
    		$informupdateList['no'] = '不接受更新通知';
    		foreach($users AS $v) {
    			$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $v->realname);
    			$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $v->account);
    			$objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $v->phone);
    			$objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $v->email);
    			$objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $membertype[$v->utype]);
    			$objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $this->loadModel('zhaopin')->getRegionsId2Name($v->regions1).' '.$this->loadModel('zhaopin')->getRegionsId2Name($v->regions2).' '.$this->loadModel('zhaopin')->getRegionsId2Name($v->regions3));
    			$objPHPExcel->getActiveSheet()->setCellValue('G' . $i, $v->address);
    			$objPHPExcel->getActiveSheet()->setCellValue('H' . $i, $v->position);
    			$objPHPExcel->getActiveSheet()->setCellValue('I' . $i, $v->company);
    			$objPHPExcel->getActiveSheet()->setCellValue('J' . $i, $v->zipcode);
    			$objPHPExcel->getActiveSheet()->setCellValue('K' . $i, $v->productserver);
    			$objPHPExcel->getActiveSheet()->setCellValue('L' . $i, $v->application);
    			$objPHPExcel->getActiveSheet()->setCellValue('M' . $i, $v->other);
    			$objPHPExcel->getActiveSheet()->setCellValue('N' . $i, $informupdateList[$v->informupdate]);
    			$objPHPExcel->getActiveSheet()->setCellValue('O' . $i, $v->last);
    			$i++;
    		}
    		$outputFileName = 'user'.time().'.xls';
    		...
    		if(rename(dirname(__FILE__).'/model'.$outputFileName, $datauplode[0].'www/data/upload/excel/'.$outputFileName)) {
    			$url .= '<a href="data/upload/excel/'.$outputFileName.'" alt="点击下载" title="点击下载">'.$outputFileName.'</a><br>';
    		}
    	}
