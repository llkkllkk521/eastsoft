...
class zhaopinModel extends model
{
    ...
    {   
        /* Get article self. */
        $article = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('alias')->eq($articleID)->fetch();
        if(!$article) $article = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('id')->eq($articleID)->fetch();
    ...
    {
        /* Get article self. */
        $page = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('alias')->eq($pageID)->andWhere('type')->eq('page')->fetch();
        if(!$page) $page = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('id')->eq($pageID)->fetch();
    ...
    public function getList($type, $categories, $orderBy, $pager = null)
    {
        ...
        if($type == 'page')
        {
            $articles = $this->dao->select('*')->from(TABLE_ZHAOPIN)
                ->where('type')->eq('page')
    ...
    {
        return $this->dao->select('id, title')->from(TABLE_ZHAOPIN)
            ->where('type')->eq('page')
            ->andWhere('addedDate')->le(helper::now())
            ->andWhere('status')->eq('normal')
            ->orderBy('id_desc')
    ...
    {
        return $this->dao->select('t1.id, t1.title, t1.alias')->from(TABLE_ZHAOPIN)->alias('t1')
    ...
    { 
        $sticks = $this->dao->select('t1.*, t2.category')->from(TABLE_ZHAOPIN)->alias('t1')
                ->leftJoin(TABLE_RELATION)->alias('t2')->on('t1.id = t2.id')
    ...
    public function getPrevAndNext($current, $category)
    {
        ...
        $prev = $this->dao->select('t1.id, title, alias')->from(TABLE_ZHAOPIN)->alias('t1')
           ->leftJoin(TABLE_RELATION)->alias('t2')->on('t1.id = t2.id')
       ...
       $next = $this->dao->select('t1.id, title, alias')->from(TABLE_ZHAOPIN)->alias('t1')
           ->leftJoin(TABLE_RELATION)->alias('t2')->on('t1.id = t2.id')
           ->where('t2.category')->eq($category)
           ->andWhere('t1.addedDate')->le(helper::now())
           ->andWhere('t1.status')->eq('normal')
    ...
    public function create($type)
    {
        ...
        $article->summary  = $this->rtrimContent($_POST['summary']);
		
        $this->dao->insert(TABLE_ZHAOPIN)
    ...
    public function forward2Blog($articleID)
    {
        ...
        $blog = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('alias')->eq($articleID)->fetch();
        if(!$blog) $blog = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('id')->eq($articleID)->fetch();
        ...
        $this->dao->insert(TABLE_ZHAOPIN)->data($blog, $skip='id,categories')->autoCheck()->batchCheck($this->config->article->require->forward2Blog, 'notempty')->exec();
    ...
    public function forward2Forum($articleID)
    {
        ...
        $thread = $this->dao->select('*')->from(TABLE_ZHAOPIN)->where('alias')->eq($articleID)->fetch();
        if(!$thread) $thread = $this->dao->select('title, content')->from(TABLE_ZHAOPIN)->where('id')->eq($articleID)->fetch();
    ...
    public function update($articleID, $type = 'article')
    {
        ...
        $this->dao->update(TABLE_ZHAOPIN)
            ->data($article, $skip = 'categories,uid,isLink')
            ->autoCheck()
            ->batchCheckIF($type == 'contribution', $this->config->zhaopin->require->post, 'notempty')
            ->batchCheckIF($type != 'page' and $type != 'contribution' and !$this->post->isLink, $this->config->zhaopin->require->edit, 'notempty')
            ->batchCheckIF($type == 'page' and !$this->post->isLink, $this->config->zhaopin->require->page, 'notempty')
            ->batchCheckIF($type != 'page' and $this->post->isLink, $this->config->zhaopin->require->link, 'notempty')
            ->batchCheckIF($type == 'page' and $this->post->isLink, $this->config->zhaopin->require->pageLink, 'notempty')
    ...
    public function delete($articleID, $null = null)
    {
        ...
        $this->dao->delete()->from(TABLE_ZHAOPIN)->where('id')->eq($articleID)->exec();
    ...
    public function setCss($articleID)
    {
        ...
        $this->dao->update(TABLE_ZHAOPIN)->data($data, $skip = 'uid')->autoCheck()->where('id')->eq($articleID)->exec();
    ...
    public function approve($articleID, $type, $categories)
    {
        ...
        $this->dao->update(TABLE_ZHAOPIN)->set('type')->eq($type)->set('contribution')->eq(2)->where('id')->eq($articleID)->exec();
        $article = $this->getByID($articleID);
    ...
    {
        $this->dao->update(TABLE_ZHAOPIN)->set('contribution')->eq(3)->where('id')->eq($articleID)->exec();
        $article = $this->getByID($articleID);
    ...
    {
        $contributions = $this->dao->select('count(*) as count')->from(TABLE_ZHAOPIN)
    ...
    public function cateList() {
    	...
    	$_regions1 = $this->dao->select('regions1')->from(TABLE_ZHAOPIN)->groupBy('regions1')->fetchAll();
    	foreach($_regions1 AS $r) {
    		$_regions_arr1[] = $r->regions1;
    	}
    	$_regions2 = $this->dao->select('regions2')->from(TABLE_ZHAOPIN)->groupBy('regions2')->fetchAll();
    	foreach($_regions2 AS $r) {
    		$_regions_arr2[] = $r->regions2;
    	}
    	$str = '<select name="region" class="region">';
    	$str .= '<option value="">全部城市</option>';
    	foreach($regions AS $r) {
    		if(in_array($r->region_id, $_regions_arr1)) {
    			
    		
    		$str .= '<option value="'.$r->region_id.'">'.$r->local_name.'</option>';
    		$children = $this->cateChildren($r->region_id);
    		foreach($children AS $c) {
    			if(in_array($c->region_id, $_regions_arr2)) {
