...
try {
	...
	$rs = $dbh->query("SELECT account, email FROM ".$config->db->prefix."user WHERE informupdate='timely'");
	$_smtps = $rs->fetchAll();
	...
	$date1 = strtotime(date('Y-m-d 00:00:00'));//当天的开始时间
	$date2 = strtotime(date('Y-m-d 23:59:59'));//当天的结束时间
	$content = '';
	if($_rsContents) {
