...
class user extends control
{
    ...
    public function register()
    {
        ...
        if(!empty($_POST))
        {
        	$_POST['realname'] = $_POST['account'];
        	$_POST['application'] = implode(',', $_POST['application']);
        ...
        $this->view->regions1 = $this->loadModel('zhaopin')->getRegions();
        $this->view->regions2 = $this->loadModel('zhaopin')->getRegions(21);
        $this->view->regions3 = $this->loadModel('zhaopin')->getRegions(22);
    ...
    public function hdyg($pageID = 1) {
    	...
    	$families   = $this->loadModel('tree')->getFamily(105, 'article');
    	$sticks     = $this->loadModel('article')->getSticks($families, 'article');
    	$articlelist   = $this->loadModel('article')->getList('article', $families, 'addedDate_desc', $pager);
    	$articlelist   = $sticks + $articlelist;
    
    	$this->view->articlelist    = $articlelist;
    	$this->view->pager      = $pager;
    	$this->view->_title      = '活动预告';
    ...
    public function zswd($pageID = 1) {
    	...
    	$families   = $this->loadModel('tree')->getFamily(103, 'article');
    	$sticks     = $this->loadModel('article')->getSticks($families, 'article');
    	$articlelist   = $this->loadModel('article')->getList('article', $families, 'addedDate_desc', $pager);
    	$articlelist   = $sticks + $articlelist;
     ...
     * 第一步调查
     */
    public function surveyonestep() {
    	$this->view->survey = $this->user->checkUserAnswer($this->app->user->id);
    	$this->display();
    }
     ...
     * 第二步
     */
    public function surveytwostep() {
    	$this->view->surveyone = str_replace('|', '', $_POST['surveyone'][0]).'|'.str_replace('|', '', $_POST['surveyone'][1]).'|'.implode(',', $_POST['myd']).'|'.str_replace('|', '', $_POST['surveyone'][2]);
    	$this->view->survey = $this->user->getSurvey($this->app->user->account);
    	$this->view->surveys = $this->user->getSurveyAll($this->app->user->account);
    	$this->display();
    }
     ...
     * 提交调查
     */
    public function savesurveycontent() {
    	$this->user->saveSurveyContentAll($this->app->user->id);
    }
