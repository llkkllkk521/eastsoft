...
class articleModel extends model
{
    ...
    /**
     * [getChildren 获取分类下的子分类，两级]
     * @AuthorName lzhan<lzhan@3ti.us>
     * @DateTime   2016-02-16T10:57:14+0800
     * @param      [type] $categoryID [description]
     * @return     [type] [description]
     */
    public function getChildren($categoryID)
    {
    $res = $this->dao->select('name, id, abbr, `order`')->from('es_category')
            ->where('type')->eq('article')
            ->andWhere('parent')->eq($categoryID)
            ->orderBy('`order` asc')->fetchAll();

        foreach ($res as $key => $value) {
            $res[$key]->sub = $this->dao->select('*')->from('es_category')
                            ->where('type')->eq('article')
                            ->andWhere('parent')->eq($value->id)->fetchAll();
        }
		foreach($res as $key => $value) {
			if($value->sub) {
				foreach($value->sub AS $k => $v) {
					$zp = $this->dao->select('name, id, abbr')->from('es_category')
					->where('type')->eq('zhaopin')
					->andWhere('name')->eq($v->name)->fetch();
					if($zp->id) {
						$res[$key]->sub[$k]->id = $zp->id;
					}
				}
			}
		}
     ...
     * [getTop 获取当前分类的顶级分类]
     * @AuthorName lzhan<lzhan@3ti.us>
     * @DateTime   2016-02-16T10:57:08+0800
    ...
    public function getTop($category) {
        ...
        if ($category->parent) {
            ...
            $cat = $this->dao->select('name, id, parent')->from('es_category')
                    ->where('type')->eq('article')
                    ->andWhere('id')->eq($category->parent)->fetch();

            if ($cat->parent) {
                return $this->getTop($cat);
            }else{
                return $cat->id;
            }
	...
	public function getRelevanceProduct($articleId) {
		$data = $this->dao->select('article, product')->from('es_article_product')
		->where('article')->eq($articleId)->fetch();
		$in = implode(',', json_decode($data->product, true));
		$products = $this->dao->select('id, name')->from(TABLE_PRODUCT)->where('id')->in($in)->fetchAll();
		return $products;
	}
	 ...
	 * @param number $categoryId 要关联类目的id
	 * @return boolean
	 */
	public function checkRelevance($articleId, $categoryId) {
		$data = $this->dao->select('id, category')->from(TABLE_RELATION)
		->where('id')->eq($articleId)->fetchAll();
		$arr = array();
		foreach($data AS $value) {
			$arr[] = $value->category;
		}
		return in_array($categoryId, $arr);
	}
}
