...
class site extends control
{
    ...
    public function setSmtp() {
    	...
    	if(!empty($_POST))
    	{
    		$setting = fixer::input('post')
    		->get();
    		$result = $this->loadModel('setting')->setItems('system.common.crontab', $setting, 'all');
    
    		if($result) $this->send(array('result' => 'success', 'message' => $this->lang->setSuccess, 'locate' => inlink('setsmtp')));
    		$this->send(array('result' => 'fail', 'message' => dao::getError()));
    	}
    
    	$this->view->title    = $this->lang->site->setSmtp;
    	$this->view->location = $location;
    	$this->display();
    }
     ...
     * @DateTime   2016-02-02T15:52:26+0800
     */
    public function carrousel($objectType = 'carrousel', $objectID = '', $isImage = null)
    {

        $this->view->title      = "<i class='icon-paper-clip'></i> " . ($isImage ? $this->lang->file->imageList : $this->lang->file->browse);
        $this->view->modalWidth = 800;
        $this->view->writeable  = $this->site->checkSavePath();
        $this->view->objectType = $objectType;
        $this->view->objectID   = $objectID;
        $this->view->files      = $this->site->getByObject($objectType, $objectID, $isImage);

        $this->view->users      = $this->loadModel('user')->getPairs();
        $this->view->title = $this->lang->site->carrousel;
        $this->display();
    }
