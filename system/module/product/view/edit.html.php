  ...
  <div class='panel-body'>
    <form method='post' id='ajaxForm'>
      <table class='table table-form'>
        ...
        <?php 
          $details = $this->dao->select('*')->from('es_product_custom')
                      ->where('product')->eq($product->id)
                      ->andWhere('label')->eq($value->id)->fetchAll();
          $detail_id = array();

          foreach ($details as $details_value) {
            $detail_id[] = $details_value->value;
          }
        ?>
        <tr>
          <th><?php echo $value->name;?></th>
          <td>
            <select name="value-<?php echo $value->id; ?>[]" class="form-control" multiple="multiple">
              <?php foreach ($value->detail as $detail) { ?>
              
              <?php $selected = (in_array($detail->id, $detail_id)) ? "selected = 'selected'" : '' ?>

              <option value="<?php echo $detail->id ?>" <?php echo $selected ?>><?php echo $detail->value ?></option>
